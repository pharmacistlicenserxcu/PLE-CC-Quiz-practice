...
# Load base pulmo items
with open('pulmo_base_items.json', 'r', encoding='utf-8') as f:
    items = json.load(f)

print(f"Total base Pulmo items: {len(items)}")

# Load all distractors_pulmo_part*.json
distractor_files = sorted(glob.glob('distractors_pulmo_part*.json'))
all_distractors = {}
...
with open('pulmo_master_pristine.json', 'w', encoding='utf-8') as f:
    json.dump(items, f, ensure_ascii=False, indent=2)

print("Saved to pulmo_master_pristine.json")
