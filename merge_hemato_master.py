...
# Load base hematologic items
with open('hemato_base_items.json', 'r', encoding='utf-8') as f:
    items = json.load(f)

print(f"Total base hematologic items: {len(items)}")

# Load all 3 distractor parts
distractors_all = {}
for p in range(1, 4):
    with open(f'distractors_hemato_part{p}.json', 'r', encoding='utf-8') as f:
...
# Clean asterisks from all fields across all items
for it in items:
    for k in list(it.keys()):
        val = str(it[k] or '')
        if '*' in val:
            it[k] = val.replace('**', '').replace('*', '').strip()
...
with open('hemato_master_pristine.json', 'w', encoding='utf-8') as f:
    json.dump(items, f, ensure_ascii=False, indent=2)

print("Saved to hemato_master_pristine.json")
