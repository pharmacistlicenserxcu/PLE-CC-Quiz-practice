...
# Load base infectious diseases items
with open('id_base_items.json', 'r', encoding='utf-8') as f:
    items = json.load(f)

print(f"Total base ID items: {len(items)}")

# Load all 6 distractor parts
distractors_all = {}
for p in range(1, 7):
    with open(f'distractors_id_part{p}.json', 'r', encoding='utf-8') as f:
...
with open('id_master_pristine.json', 'w', encoding='utf-8') as f:
    json.dump(items, f, ensure_ascii=False, indent=2)

print("Saved to id_master_pristine.json")
