...
# Load master pristine cardio questions
with open('cardio_master_pristine.json', 'r', encoding='utf-8') as f:
    items = json.load(f)

print(f"Loaded {len(items)} pristine cardio items.")

wb = openpyxl.load_workbook('PLE CC QUIZ.xlsx')
ws = wb['2. Cardiovascular']

# Check banner in Row 1
...
for idx, it in enumerate(items, 1):
    ...
    # Col B: คำถาม
    ws.cell(row=row_num, column=2, value=it['q_text']).alignment = align_left
    ws.cell(row=row_num, column=2).font = font_regular
    
    # Col C: รูปถาม
    ws.cell(row=row_num, column=3, value=it['q_img']).alignment = align_center
    ...
    # Col I: เฉลย (1-5)
    ws.cell(row=row_num, column=9, value=it['ans']).alignment = align_center
    ws.cell(row=row_num, column=9).font = font_regular
    
    # Col J: คำอธิบายเฉลย
    ws.cell(row=row_num, column=10, value=it['exp']).alignment = align_left
    ws.cell(row=row_num, column=10).font = font_regular
    
    # Col K: รูปเฉลย
    ws.cell(row=row_num, column=11, value=it['exp_img']).alignment = align_center
    ws.cell(row=row_num, column=11).font = font_regular
    
    # Col L: Filter หมวด/Subtopic
    ws.cell(row=row_num, column=12, value=it['subtopic']).alignment = align_left
    ...
    # Col N: หมายเหตุ
    ws.cell(row=row_num, column=14, value=it['note']).alignment = align_left
    ws.cell(row=row_num, column=14).font = font_regular
    
    # Col O: ประเภทข้อสอบ
    ws.cell(row=row_num, column=15, value=it['q_type']).alignment = align_center
    ws.cell(row=row_num, column=15).font = font_regular
    
    # Col P: ปี / เลขชุด
    ws.cell(row=row_num, column=16, value=it['exam_year']).alignment = align_center
    ws.cell(row=row_num, column=16).font = font_regular

print(f"Successfully wrote {len(items)} rows to 2. Cardiovascular.")
